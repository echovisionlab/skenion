...
Do not use manual package version bumps to describe compatibility. Let Release Please produce versions from Conventional Commits.
...
If a repository consumes a newly published contract surface, update the compatibility matrix and use an explicit release note in the consuming repository.
